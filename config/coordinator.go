...
// `Reload()`.
func NewRealTimeCoordinator(configFilePath string,receiverFilePath string , r prometheus.Registerer, l log.Logger) *Coordinator {
	c := &Coordinator{
...
func (c *Coordinator) loadFromFile() error {
	...
	conf, plainConfig, err := LoadFile(c.configFilePath,c.ReceiverFilePath)
	if err != nil {
